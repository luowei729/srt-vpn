...
### 改动前总结
- **用户凌晨实测（北京时间约 01:22）**：v0.5.4 多线 WebRTC 上传仍卡死为 0、单线上传仅 4Mbps；关键观察：**speedtest 切到上传阶段时，SG 网卡仍有 150M 发送流量在跑**——下载阶段的残留流量没有停止，用户准确推断其挤占了上行。
- **对照实验证实**（自写 SOCKS5 UDP ASSOCIATE 打流脚本 + SG tcpdump 计数）：单独上行到达 ≈66Mbps（75452 包/窗口）；并发 100MB 下载时掉到 ≈38Mbps（43735 包，**↓42%**）。
- **根因**：客户端接收通道为 unbounded——浏览器停止读下载（测速切换阶段）后上层消费者停取数据，但接收线程仍全速收包入队 → SRT 流控永不刹车 → 服务端持续发送无人消费的残留流量 + 海量 ACK/NACK 洪水，与上行测试流量抢占窄上行。

### 改动后总结
1. **接收通道高水位背压**（connection.rs `run_recv_loop`）：新增原子积压计数器（recv 线程入队 +1 / recv·try_recv·recv_async·recv_batch_async 消费 -1），积压 >4096 条暂停 epoll 收包、<1024 恢复 → SRT 内核缓冲涨满触发流控刹车 → 发送方自动减速，残留下载快速消散。
2. **SNDBUF 32MB→8MB**：最坏残留量降为 1/4（8MB 仍远超公网 BDP≈1.75MB，吞吐无影响）。
3. **复验**：并发下载时上行到达 **43,735 → 146,766 包（3.4 倍）**；cargo test 32/32。
4. 遗留观察项：WebRTC 打洞握手若仍偶发失败，下一轮排查 STUN 包路径与 UDP 会话超时参数。

## [2026-08-23 18:05] - v0.5.4 修复多线测速卡死（移除误判背压）+ 默认 UDP 走优先队列可靠传输

### 改动前总结
- **用户实测**：v0.5.3 部署后"测速就卡死"。
- **根因（复现定位）**：v0.5.3 的 TCP 背压判定 `msSndBuf>300ms` 在公网大 BDP 管道是常态值（85Mbps×70ms RTT 在途数据即数百 ms，speedtest 8 并发共享一条 SRT 连接时稳态远超）→ 判定恒真 → 发送线程持续跳过可靠队列 → 全部 TCP 流饿死 = 卡死。单流短传 msSndBuf 未达稳态所以本机复测正常（又一次回环/单流验证盲区）。

### 改动后总结
1. **移除 TCP 背压分支**（connection.rs）：绝对时间跨度不能作积压异常判据；如需背压应基于 byteAvailSndBuf 剩余比例另行实验。保留双队列 + 优先队列插队调度。
2. **默认 UDP 走"优先队列 + 可靠语义"**（dispatch.rs + connection.rs 新增 `send_priority()`）：UDP 编 Data 帧（msgttl=-1/inorder=1 必达，保 WebRTC STUN/DTLS 存活）但投优先队列——发送线程每轮先清空它，UDP 不被 TCP 积压拖延。可靠性（v0.5.2 教训）与低排队延时（v0.5.3 调度能力）首次兼得。
3. **验证**：cargo test 32/32；回环下行 92.8MB/s 持平基线、UDP DNS PASS。
